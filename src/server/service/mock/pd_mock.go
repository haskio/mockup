...
// Author [piexlmax](https://github.com/piexlmax)
func (pdMockService *PdMockService) GetPdMockInfoList(uid uint, info mockReq.PdMockSearch) (list []mock.PdMock, total int64, err error) {
	limit := info.PageSize
	...
	if uid == 1 {
		...
		if info.StartCreatedAt != nil && info.EndCreatedAt != nil {
			db = db.Where("created_at BETWEEN ? AND ? Uid =? ", info.StartCreatedAt, info.EndCreatedAt, uid)
		}
		if info.Title != "" {
			db = db.Where("title LIKE ?  AND Uid =? ", "%"+info.Title+"%", uid)
		}
		err = db.Where("Uid =?", uid).Count(&total).Error
	}
